...
namespace BankApp.Controllers
{
    ...
    public class EmployeeController : Controller
    {
        ...
        public async Task<IActionResult> MakeDecision(int offerID, int offerIdInBank, string email, bool decision)
        {
            var user = await _userManager.FindByNameAsync(User.Identity.Name);
            ...
            if (decision)
            {
                ...
                {
                    case "projectAPI":
                        _MiNIClient.CompleteOfferAsync(offerIdInBank);
                        break;
                    case "BestWorldAPI":
                        _BestAPIClient.CompleteOfferAsync(offerIdInBank);
                        break;
                    case "StrangerAPI":
                        _StrangerClient.CompleteOfferAsync(offerIdInBank);
                        break;
                }
            }
